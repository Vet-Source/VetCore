  ...
  return (
    <div style={{ padding: 32 }}>
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ color: "#6b7280", fontSize: 14, marginTop: 4 }}>VET-SOURCE blockchain claims platform</p>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", overflow: "hidden" }}>
        <div style={{ padding: "20px 24px", borderBottom: "1px solid #f3f4f6", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#111827", margin: 0 }}>Recent Claims</h2>
          <Link href="/dashboard/submit" style={{ fontSize: 13, color: "#4d9e6e", fontWeight: 600, textDecoration: "none", padding: "7px 14px", border: "1px solid #4d9e6e", borderRadius: 8 }}>+ Submit new claim</Link>
        </div>
        {loading ? (
          ...
          <div style={{ padding: 48, textAlign: "center" }}>
            ...
            <Link href="/dashboard/submit" style={{ display: "inline-block", marginTop: 16, padding: "10px 20px", background: "#4d9e6e", color: "#fff", borderRadius: 8, textDecoration: "none", fontSize: 14, fontWeight: 600 }}>Submit Claim</Link>
          </div>
        ) : (
            ...
            <tbody>
              {claims.slice(0, 10).map((claim, i) => (
                <tr key={claim.id} style={{ borderTop: "1px solid #f3f4f6", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                  ...
                  <td style={{ padding: "14px 20px", fontSize: 13, fontWeight: 600, color: "#059669" }}>£{Number(claim.amount)?.toLocaleString()}</td>
                  ...
                  <td style={{ padding: "14px 20px" }}>
                    <Link href={`/dashboard/claims/${claim.id}`} style={{ fontSize: 13, color: "#4d9e6e", fontWeight: 600, textDecoration: "none" }}>View →</Link>
                  </td>
                </tr>
              ))}
            </tbody>
