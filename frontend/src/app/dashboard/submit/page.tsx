    ...
    <div style={{ padding: 32, maxWidth: 800, margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24, marginBottom: 20 }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#374822", marginTop: 0, marginBottom: 20 }}>ðŸ¾ Patient Information</h2>
        ...
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24, marginBottom: 20 }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#374822", marginTop: 0, marginBottom: 20 }}>ðŸ’Š Treatment Details</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div><label style={lbl}>Claim Amount (Â£) *</label><input name="amount" type="number" min="0" step="0.01" value={form.amount} onChange={handleChange} required style={inp} placeholder="0.00" /></div>
        ...
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24, marginBottom: 20 }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#374822", marginTop: 0, marginBottom: 20 }}>ðŸ¥ Clinic Information</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24, marginBottom: 24 }}>
          <h2 style={{ fontSize: 16, fontWeight: 600, color: "#374822", marginTop: 0, marginBottom: 8 }}>ðŸ“Ž Supporting Documents</h2>
          ...
          <div onClick={() => fileInputRef.current?.click()} style={{ border: "2px dashed #95c11f", borderRadius: 10, padding: "28px 20px", textAlign: "center", cursor: "pointer", background: files.length ? "#f4fce8" : "#fafafa" }}>
            ...
            {files.length > 0 && <div style={{ marginTop: 12 }}>{files.map(f => <div key={f.name} style={{ fontSize: 12, color: "#95c11f" }}>âœ“ {f.name}</div>)}</div>}
          </div>
        ...
        <div style={{ display: "flex", gap: 12 }}>
          <button type="submit" disabled={loading} style={{ flex: 1, padding: "14px 24px", background: loading ? "#779451" : "#95c11f", color: "#374822", border: "none", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: loading ? "not-allowed" : "pointer" }}>
            {loading ? "Submitting to blockchain..." : "Submit Claim â†’"}
          </button>
