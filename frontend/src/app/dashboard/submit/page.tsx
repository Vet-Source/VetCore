...
export default function SubmitClaimPage() {
  ...
  const sec: any = { background: "#fff", borderRadius: 12, border: "1px solid #e8eaed", padding: 24, marginBottom: 20 };
  const hdg: any = { fontSize: 16, fontWeight: 600, color: "#374822", marginTop: 0, marginBottom: 20, paddingBottom: 12, borderBottom: "1px solid #f3f4f6" };
  ...
  return (
    <div style={{ padding: 32, maxWidth: 800, margin: "0 auto" }}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={sec}>
          <h2 style={hdg}>Patient Information</h2>
        ...
        <div style={sec}>
          <h2 style={hdg}>Treatment Details</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            ...
            <div><label style={lbl}>Claim Amount (GBP) *</label><input name="amount" type="number" min="0" step="0.01" value={form.amount} onChange={handleChange} required style={inp} placeholder="0.00" /></div>
        ...
        <div style={sec}>
          <h2 style={hdg}>Clinic Information</h2>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={sec}>
          <h2 style={hdg}>Supporting Documents</h2>
          <p style={{ fontSize: 13, color: "#779451", marginBottom: 16, marginTop: 0 }}>Upload invoices, medical records, or other supporting files (PDF, JPG, PNG)</p>
          <div onClick={() => fileInputRef.current?.click()} style={{ border: "2px dashed #95c11f", borderRadius: 10, padding: "28px 20px", textAlign: "center", cursor: "pointer", background: files.length ? "#f4fce8" : "#fafafa" }}>
            <div style={{ fontSize: 14, fontWeight: 600, color: "#374822", marginBottom: 4 }}>{files.length ? `${files.length} file(s) selected` : "Click to upload documents"}</div>
            <div style={{ fontSize: 12, color: "#779451" }}>PDF, JPG, PNG up to 10MB each</div>
